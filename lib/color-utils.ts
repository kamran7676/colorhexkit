...
// Basic approximation for OKLCH since full implementation is complex without a library
export function hexToOklch(hex: string): { l: number; c: number; h: number } | null {
  ...
  const C = Math.sqrt(
    Math.pow(1.9779984951 * l_ - 2.4285922050 * m_ + 0.4505937099 * s_, 2) +
    Math.pow(0.0259040371 * l_ + 0.7827717662 * m_ - 0.8086757660 * s_, 2)
  );
  let H = Math.atan2(
